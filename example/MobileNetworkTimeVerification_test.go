...
func TestMobileNetworkTimeVerification(t *testing.T) {
	...
	request := tencentcloudsdk.NewMobileNetworkTimeVerificationRequest()
	_ = json.Unmarshal([]byte(reqJson), request)

	// Step 3. 调用接口
	response, err := client.MobileNetworkTimeVerification(request)
	if err != nil {
